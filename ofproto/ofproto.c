...
int ofproto_create(const char *datapath_name, const char *datapath_type,
 ...
 * details. */
void ofproto_init_tables(struct ofproto *ofproto, int n_tables)
{
...
void ofproto_init_max_ports(struct ofproto *ofproto, uint16_t max_ports)
{
...
void ofproto_set_datapath_id(struct ofproto *p, uint64_t datapath_id)
{
...
void ofproto_set_controllers(struct ofproto *p,
...
void ofproto_set_fail_mode(struct ofproto *p, enum ofproto_fail_mode fail_mode)
{
 ...
 * them to reconnect. */
void ofproto_reconnect_controllers(struct ofproto *ofproto)
{
...
void ofproto_set_extra_in_band_remotes(struct ofproto *ofproto,
...
void ofproto_set_in_band_queue(struct ofproto *ofproto, int queue_id)
{
...
void ofproto_set_forward_bpdu(struct ofproto *ofproto, bool forward_bpdu)
{
 ...
 * 'max_entries'. */
void ofproto_set_mac_table_config(struct ofproto *ofproto, unsigned idle_time,
                                  size_t max_entries)
{
...
void ofproto_type_set_config(const char *datapath_type, const struct smap *cfg)
{
    const struct ofproto_class *class;
...
void ofproto_set_threads(int n_handlers_, int n_revalidators_)
{
...
void ofproto_set_dp_desc(struct ofproto *p, const char *dp_desc)
{
    free(p->dp_desc);
...
int ofproto_set_netflow(struct ofproto *ofproto,
...
int ofproto_set_sflow(struct ofproto *ofproto,
...
int ofproto_set_ipfix(struct ofproto *ofproto,
 ...
 *
 * Returns 0 if successful, otherwise a positive errno value. */
int ofproto_set_stp(struct ofproto *ofproto,
 ...
 *
 * Returns 0 if successful, otherwise a positive errno value. */
int ofproto_get_stp_status(struct ofproto *ofproto,
 ...
 *
 * Returns 0 if successful, otherwise a positive errno value.*/
int ofproto_port_set_stp(struct ofproto *ofproto, ofp_port_t ofp_port,
 ...
 *
 * Returns 0 if successful, otherwise a positive errno value.*/
int ofproto_port_get_stp_status(struct ofproto *ofproto, ofp_port_t ofp_port,
 ...
 *
 * Returns 0 if successful, otherwise a positive errno value.*/
int ofproto_port_get_stp_stats(struct ofproto *ofproto, ofp_port_t ofp_port,
 ...
 *
 * Returns 0 if successful, otherwise a positive errno value. */
int ofproto_set_rstp(struct ofproto *ofproto,
 ...
 *
 * Returns 0 if successful, otherwise a positive errno value. */
int ofproto_get_rstp_status(struct ofproto *ofproto,
 ...
 *
 * Returns 0 if successful, otherwise a positive errno value.*/
int ofproto_port_set_rstp(struct ofproto *ofproto, ofp_port_t ofp_port,
 ...
 *
 * Returns 0 if successful, otherwise a positive errno value.*/
int ofproto_port_get_rstp_status(struct ofproto *ofproto, ofp_port_t ofp_port,
 ...
 *
 * Returns 0 if successful, otherwise a positive errno value. */
int ofproto_port_set_queues(struct ofproto *ofproto, ofp_port_t ofp_port,
...
/* LLDP configuration. */
void ofproto_port_set_lldp(struct ofproto *ofproto,
                           ofp_port_t ofp_port,
...
int ofproto_set_aa(struct ofproto *ofproto, void *aux OVS_UNUSED,
...
void ofproto_port_clear_cfm(struct ofproto *ofproto, ofp_port_t ofp_port)
{
    struct ofport *ofport = ofproto_get_port(ofproto, ofp_port);
...
void ofproto_port_set_cfm(struct ofproto *ofproto, ofp_port_t ofp_port,
...
void ofproto_port_set_bfd(struct ofproto *ofproto, ofp_port_t ofp_port,
                          const struct smap *cfg)
{
    struct ofport *ofport;
    int error;
...
bool ofproto_port_bfd_status_changed(struct ofproto *ofproto, ofp_port_t ofp_port)
{
    struct ofport *ofport = ofproto_get_port(ofproto, ofp_port);
...
int ofproto_port_get_bfd_status(struct ofproto *ofproto, ofp_port_t ofp_port,
                                struct smap *status)
{
    struct ofport *ofport = ofproto_get_port(ofproto, ofp_port);
...
int ofproto_port_is_lacp_current(struct ofproto *ofproto, ofp_port_t ofp_port)
{
    struct ofport *ofport = ofproto_get_port(ofproto, ofp_port);
...
int ofproto_port_get_lacp_stats(const struct ofport *port, struct lacp_slave_stats *stats)
{
...
struct asp_map *init_map(struct asp_map *kv)
{
    if (kv == NULL)
    {
        int needed_size = asp_map_entry_count * sizeof(struct asp_map);
...
int set_my_asp_map(struct asp_map new_map)
{
    ...
    uint64_t _dpid = dpid % asp_map_entry_count;

    while (my_asp_map[_dpid].dpid != 0 && my_asp_map[_dpid].dpid != dpid)
    {
        ...
        _dpid = next_dpid % asp_map_entry_count;
    }

    my_asp_map[_dpid] = new_map;
...
struct asp_map *get_my_asp_map(uint64_t dpid)
{
    ...
    if (my_asp_map[_dpid].dpid == dpid)
    {
        // Find directly
        return &my_asp_map[_dpid];
    }
 ...
 * port. */
int ofproto_bundle_register(struct ofproto *ofproto, void *aux,
...
int ofproto_bundle_unregister(struct ofproto *ofproto, void *aux)
{
...
int ofproto_mirror_register(struct ofproto *ofproto, void *aux,
...
int ofproto_mirror_unregister(struct ofproto *ofproto, void *aux)
{
 ...
 */
int ofproto_mirror_get_stats(struct ofproto *ofproto, void *aux,
 ...
 * port. */
int ofproto_set_flood_vlans(struct ofproto *ofproto, unsigned long *flood_vlans)
{
 ...
 * output for a mirror. */
bool ofproto_is_mirror_output_bundle(const struct ofproto *ofproto, void *aux)
{
...
int ofproto_get_n_tables(const struct ofproto *ofproto)
{
...
void ofproto_configure_table(struct ofproto *ofproto, int table_id,
...
bool ofproto_has_snoops(const struct ofproto *ofproto)
{
    return connmgr_has_snoops(ofproto->connmgr);
}

void ofproto_get_snoops(const struct ofproto *ofproto, struct sset *snoops)
{
...
void ofproto_destroy(struct ofproto *p, bool del)
    OVS_EXCLUDED(ofproto_mutex)
{
...
void ofproto_wait(struct ofproto *p)
{
...
void ofproto_type_get_memory_usage(const char *datapath_type, struct simap *usage)
{
    const struct ofproto_class *class;
...
void ofproto_get_ofproto_controller_info(const struct ofproto *ofproto,
                                         struct shash *info)
{
    connmgr_get_controller_info(ofproto->connmgr, info);
}

void ofproto_free_ofproto_controller_info(struct shash *info)
{
    connmgr_free_controller_info(info);
}

/* Makes a deep copy of 'old' into 'port'. */
void ofproto_port_clone(struct ofproto_port *port, const struct ofproto_port *old)
{
 ...
 * ofproto_port. */
void ofproto_port_destroy(struct ofproto_port *ofproto_port)
{
 ...
 */
void ofproto_port_dump_start(struct ofproto_port_dump *dump,
 ...
 * ofproto_port_dump_done(). */
bool ofproto_port_dump_next(struct ofproto_port_dump *dump,
...
int ofproto_port_dump_done(struct ofproto_port_dump *dump)
{
    const struct ofproto *ofproto = dump->ofproto;
    if (!dump->error)
    {
...
int ofproto_port_query_by_name(const struct ofproto *ofproto, const char *devname,
                               struct ofproto_port *port)
{
    int error;
 ...
 *
 * This function has no effect if 'ofproto' does not have a port 'ofp_port'. */
void ofproto_port_set_config(struct ofproto *ofproto, ofp_port_t ofp_port,
                             const struct smap *cfg)
{
    struct ofport *ofport;
    int error;
...
void ofproto_add_flow(struct ofproto *ofproto, const struct match *match,
                      int priority,
                      const struct ofpact *ofpacts, size_t ofpacts_len)
    OVS_EXCLUDED(ofproto_mutex)
{
 ...
 *
 * This is a helper function for in-band control and fail-open. */
void ofproto_delete_flow(struct ofproto *ofproto,
...
void ofproto_flush_flows(struct ofproto *ofproto)
{
...
alloc_ofp_port(struct ofproto *ofproto, const char *netdev_name)
{
    ...
    if (port_idx >= ofproto->max_ports || ofport_get_usage(ofproto, u16_to_ofp(port_idx)) == LLONG_MAX)
    {
...
{
    return (eth_addr_equals(a->hw_addr, b->hw_addr) && eth_addr64_equals(a->hw_addr64, b->hw_addr64) && a->state == b->state && !((a->config ^ b->config) & OFPUTIL_PC_PORT_DOWN) && a->curr == b->curr && a->advertised == b->advertised && a->supported == b->supported && a->peer == b->peer && a->curr_speed == b->curr_speed && a->max_speed == b->max_speed);
}
...
ofport_modified(struct ofport *port, struct ofputil_phy_port *pp)
{
    ...
    port->pp.config = ((port->pp.config & ~OFPUTIL_PC_PORT_DOWN) | (pp->config & OFPUTIL_PC_PORT_DOWN));
    port->pp.state = ((port->pp.state & ~OFPUTIL_PS_LINK_DOWN) | (pp->state & OFPUTIL_PS_LINK_DOWN));
...
void ofproto_port_set_state(struct ofport *port, enum ofputil_port_state state)
{
...
int ofproto_port_get_stats(const struct ofport *port, struct netdev_stats *stats)
{
    struct ofproto *ofproto = port->ofproto;
    int error;
...
{
    return ofport_is_internal_or_patch(p, port) && !netdev_mtu_is_user_config(port->netdev);
}
    ...
    OVS_NO_THREAD_SAFETY_ANALYSIS
{
    ...
    if (rule->flags & OFPUTIL_FF_SEND_FLOW_REM && rule->removed_reason != OVS_OFPRR_NONE && !rule_is_hidden(rule))
    {
...
void ofproto_rule_ref(struct rule *rule)
{
    if (rule)
    {
...
bool ofproto_rule_try_ref(struct rule *rule)
{
    if (rule)
    {
...
void ofproto_rule_unref(struct rule *rule)
{
...
void ofproto_group_ref(struct ofgroup *group)
{
    if (group)
    {
...
bool ofproto_group_try_ref(struct ofgroup *group)
{
    if (group)
    {
...
void ofproto_group_unref(struct ofgroup *group)
    OVS_NO_THREAD_SAFETY_ANALYSIS
{
...
rule_actions_create(const struct ofpact *ofpacts, size_t ofpacts_len)
{
    ...
    actions->has_groups =
        (ofpact_find_type_flattened(ofpacts, OFPACT_GROUP,
                                    ofpact_end(ofpacts, ofpacts_len)) != NULL);
    actions->has_learn_with_delete = (next_learn_with_delete(actions, NULL) != NULL);

    return actions;
}

/* Free the actions after the RCU quiescent period is reached. */
void rule_actions_destroy(const struct rule_actions *actions)
{
    if (actions)
    {
...
bool ofproto_rule_has_out_port(const struct rule *rule, ofp_port_t port)
    OVS_REQUIRES(ofproto_mutex)
{
    if (port == OFPP_ANY)
    {
...
handle_get_config_request(struct ofconn *ofconn, const struct ofp_header *oh)
{
    ...
    config.invalid_ttl_to_controller = ofconn_get_invalid_ttl_to_controller(ofconn);
...
handle_set_config(struct ofconn *ofconn, const struct ofp_header *oh)
{
    ...
    if (ofconn_get_type(ofconn) != OFCONN_PRIMARY || ofconn_get_role(ofconn) != OFPCR12_ROLE_SLAVE)
    {
    ...
    OVS_REQUIRES(ofproto_mutex)
{
    ...
    OFPACT_FOR_EACH_FLATTENED(a, ofpacts, ofpacts_len)
    {
        ...
        if (a->type == OFPACT_GROUP && !ofproto_group_exists(ofproto, ofpact_get_GROUP(a)->group_id))
        {
...
void ofproto_packet_out_uninit(struct ofproto_packet_out *opo)
{
                   ...
                   enum ofputil_port_config mask)
{
    ...
    if (toggle & OFPUTIL_PC_PORT_DOWN && (config & OFPUTIL_PC_PORT_DOWN
...
collect_rule(struct rule *rule, const struct rule_criteria *c,
    ...
    OVS_REQUIRES(ofproto_mutex)
{
    if ((c->table_id == rule->table_id || c->table_id == 0xff) && ofproto_rule_has_out_port(rule, c->out_port) && ofproto_rule_has_out_group(rule, c->out_group) && !((rule->flow_cookie ^ c->cookie) & c->cookie_mask) && (!rule_is_hidden(rule) || c->include_hidden) && cls_rule_visible_in_version(&rule->cr, c->version))
    {
...
void ofproto_get_all_flows(struct ofproto *p, struct ds *results)
{
    struct oftable *table;
...
void ofproto_get_netflow_ids(const struct ofproto *ofproto,
...
bool ofproto_port_cfm_status_changed(struct ofproto *ofproto, ofp_port_t ofp_port)
{
    struct ofport *ofport = ofproto_get_port(ofproto, ofp_port);
...
int ofproto_port_get_cfm_status(const struct ofproto *ofproto, ofp_port_t ofp_port,
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    table = &ofproto->tables[table_id];
    if (table->flags & OFTABLE_READONLY && !(fm->flags & OFPUTIL_FF_NO_READONLY))
    {
        return OFPERR_OFPBRC_EPERM;
    }

    if (!(fm->flags & OFPUTIL_FF_HIDDEN_FIELDS) && !match_has_default_hidden_fields(&fm->match))
    {
    ...
    OVS_REQUIRES(ofproto_mutex)
{
    ...
    if (!old_rule)
    {
        ...
        if (new_rule->flags & OFPUTIL_FF_CHECK_OVERLAP && classifier_rule_overlaps(&table->cls, &new_rule->cr,
                                                                                   ofm->version))
        {
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
     ...
     * action. */
    if (fm->command != OFPFC_MODIFY_STRICT || fm->table_id == OFPTT_ALL || fm->flags & OFPUTIL_FF_RESET_COUNTS || fm->buffer_id != UINT32_MAX)
    {
        return OFPERR_OFPFMFC_UNKNOWN;
    }
    ...
    if (rule)
    {
        ...
        ovs_mutex_lock(&rule->mutex);
        actions = rule_get_actions(rule);
        if (rule->idle_timeout == fm->idle_timeout && rule->hard_timeout == fm->hard_timeout && rule->importance == fm->importance && rule->flags == (fm->flags & OFPUTIL_FF_STATE) && (!fm->modify_cookie || (fm->new_cookie == rule->flow_cookie)) && ofpacts_equal(fm->ofpacts, fm->ofpacts_len, actions->ofpacts, actions->ofpacts_len))
        {
                   ...
                   struct rule *old_rule, struct rule *new_rule)
{
    ...
    if (old_rule)
    {
        ...
        {
            bool change_cookie = (ofm->modify_cookie && new_rule->flow_cookie != OVS_BE64_MAX && new_rule->flow_cookie != old_rule->flow_cookie);
            ...
            if (!change_cookie)
            {
                *CONST_CAST(ovs_be64 *, &new_rule->flow_cookie) = old_rule->flow_cookie;
            }
    ...
    OVS_REQUIRES(ofproto_mutex)
{
    ...
    if (old_rule)
    {
        ...
        if (replaced_rule)
        {
            ...
            bool changed_cookie = (new_rule->flow_cookie != old_rule->flow_cookie);
    ...
    OVS_REQUIRES(ofproto_mutex)
{
    ...
    if (rule_collection_n(old_rules) > 0)
    {
        ...
        ovs_assert(rule_collection_n(new_rules) == rule_collection_n(old_rules));
    ...
    OVS_REQUIRES(ofproto_mutex)
{
    ...
    if (rule_collection_n(old_rules) == 0 && rule_collection_n(new_rules) == 1)
    {
...
void ofproto_rule_reduce_timeouts__(struct rule *rule,
                                    uint16_t idle_timeout, uint16_t hard_timeout)
    OVS_REQUIRES(ofproto_mutex)
...
void ofproto_rule_reduce_timeouts(struct rule *rule,
                                  uint16_t idle_timeout, uint16_t hard_timeout)
    ...
    if ((fm->command == OFPFC_DELETE || fm->command == OFPFC_DELETE_STRICT) && fm->table_id == 255 && fm->priority == 65535)
    {
        if (fm->command == OFPFC_DELETE && req->request->xid == current_map->xid)
        {
            ...
            else
            {
                ...
                error = ofproto_flow_mod_start(ofproto, current_map->ofm);
                if (!error)
                {
                    ofproto_bump_tables_version(ofproto);
                    ofproto_flow_mod_finish(ofproto, current_map->ofm, req);
                    ofmonitor_flush(ofproto->connmgr);
                }
        ...
        else if (fm->command == OFPFC_DELETE_STRICT && req->request->xid == current_map->xid)
        {
            ...
            {
                ofproto_flow_mod_uninit(current_map->ofm);
                free(current_map->ofm);
            }
...
handle_role_request(struct ofconn *ofconn, const struct ofp_header *oh)
{
    ...
    {
        if (request.role != OFPCR12_ROLE_EQUAL && request.have_generation_id && !ofconn_set_master_election_id(ofconn, request.generation_id))
        {
...
void ofmonitor_collect_resume_rules(struct ofmonitor *m,
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    for (;;)
    {
        ...
        if (request.table_id != 0xff && request.table_id >= ofproto->n_tables)
        {
            error = OFPERR_OFPBRC_BAD_TABLE_ID;
            goto error;
        }
                    ...
                    enum ofptype type, struct ovs_list *replies)
{
    ...
    if (type == OFPTYPE_METER_STATS_REQUEST)
    {
        ...
        ofpbuf_clear(&bands);
        stats.bands = ofpbuf_put_uninit(&bands, meter->n_bands * sizeof *stats.bands);
    ...
    { /* type == OFPTYPE_METER_CONFIG_REQUEST */
                       ...
                       ovs_version_t version)
{
    ...
    CMAP_FOR_EACH_WITH_HASH(group, cmap_node, hash_int(group_id, 0),
                            &ofproto->groups)
    {
        if (group->group_id == group_id && versions_visible_in_version(&group->versions, version))
        {
            return group;
        }
    }
    ...
    OVS_REQUIRES(ofproto_mutex)
{
    ...
    if (ofproto->n_groups[ogm->gm.type] >= ofproto->ogf.max_groups[ogm->gm.type])
    {
        return OFPERR_OFPGMFC_OUT_OF_GROUPS;
    }
    ...
    OVS_REQUIRES(ofproto_mutex)
{
    ...
    if (old_group->type != ogm->gm.type && (ofproto->n_groups[ogm->gm.type] >= ofproto->ogf.max_groups[ogm->gm.type]))
    {
        return OFPERR_OFPGMFC_OUT_OF_GROUPS;
    }
 ...
 * function. */
void ofproto_group_delete_all(struct ofproto *ofproto)
    OVS_EXCLUDED(ofproto_mutex)
{
    struct ofproto_group_mod ogm;
...
table_mod(struct ofproto *ofproto, const struct ofputil_table_mod *tm)
{
     ...
     * defined". */
    if (tm->eviction_flags != UINT32_MAX && tm->eviction_flags != OFPROTO_EVICTION_FLAGS)
    {
...
void ofproto_flow_mod_uninit(struct ofproto_flow_mod *ofm)
{
    if (ofm->temp_rule)
    {
        ofproto_rule_unref(ofm->temp_rule);
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    ofm->modify_may_add_flow = (fm->new_cookie != OVS_BE64_MAX && fm->cookie_mask == htonll(0));
    ...
    else
    {
        ...
        if (error)
        {
            ...
            LIST_FOR_EACH_REVERSE_CONTINUE(be, node, &bundle->msg_list)
            {
                if (be->type == OFPTYPE_FLOW_MOD)
                {
                    VLOG_WARN("My: Revert: CommitBID(%d) and currentBID(%d)\n", bundle->id, current_map->bid);
                    if (bundle->id == current_map->bid)
                    {
                        ofproto_flow_mod_revert(ofproto, current_map->ofm);
                    }
                    else
                    {
        ...
        else
        {
            ...
            LIST_FOR_EACH(be, node, &bundle->msg_list)
            {
                ...
                else if (!(bundle->id == current_map->bid))
                {
                    version =
            ...
            if (is_asp)
            {
                ...
                else
                {
                    ...
                    if (oh->xid == current_map->xid)
                    {
                        ...
                        error = ofproto_flow_mod_init(ofproto, current_map->ofm, &fm, NULL);
                    }
                }
            }
            else
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    if (oh->version >= OFP13_VERSION && ofpmsg_is_stat_request(oh) && ofpmp_more(oh))
    {
    ...
    OVS_REQUIRES(ofproto_mutex)
{
    ...
    if ((table->eviction != 0) == (eviction != 0) && n_fields == table->n_eviction_fields && (!n_fields || !memcmp(fields, table->eviction_fields, n_fields * sizeof *fields)))
    {
